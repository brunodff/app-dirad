...
export default function SDAPHome() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.rpc("assign_role", {
        target_email: clean, new_role: "SDAP"
      });
      if (error) throw error;
      setMsg(`✅ ${clean} agora é SDAP.`);
      setEmail("");
    } catch (err: any) {
  ...
  return (
    <>
      <Topbar role="SDAP" onLogout={handleLogout} />

      <main className="section">
        <div className="card" style={{ maxWidth: 900, margin: "0 auto", padding: 18 }}>
          <h1>SDAP</h1>
          <p>Adicionar novos usuários SDAP e acessar os painéis do setor.</p>
          ...
          <form onSubmit={submit} className="grid-form" style={{ marginTop: 12 }}>
            ...
            <div />
            <div><button className="btn pill" type="submit" style={{ marginTop: 6 }}>Adicionar SDAP</button></div>
          </form>
